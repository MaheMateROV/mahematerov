...
sock.sendto("1,2,2,1,7,3".encode(),("CODE IP HERE",1234))#PC whose ip given in ARD2IP
"""

#Testing mode generates random values instead reading from serial port
TESTING=False
#STRING IP ADDRESS OF ARDUINOS
ARD1IP="10.8.1.1757"
ARD2IP="10.8.1.175"
SOCKTIMEOUT = 0
...
def listen_for_packets():
    while True:
        try:
            ...
            elif(addr[0]==ARD2IP):updateButton(buttons,arr,6)
...
try:  
    ...
    def updateButton(buttons,val,ard=8):
        if ard==8:
            ...
            buttons[N].config(text=f"Temp: {val[-1]}")

    # Function to update last 4 button texts

    style.configure('success.Outline.TButton', background='black', foreground='white',font=('Arial', 20))

        
    def getButton(i,j,cs=1):
        button = ttk.Button(root, text="0", style="success.Outline.TButton")
        button.grid(row=i, column=j, sticky="nsew", padx=5, pady=5,columnspan=cs)
    ...
    for i in range(3):
        for j in range(4):
            buttons.append(getButton(i,j))
    buttons.append(getButton(3,0,2))
    buttons.append(getButton(3,2,2))    
